...
const Login = () => {
    ...
    const loginHandling = async () => {
        ...
        const passInpt = document.getElementById("password").value;

        if(emailInpt == "" || passInpt == ""){
            if (emailInpt == ""){
                setIsEmailErr(true);
            } else {
                setIsEmailErr(false);
    ...
    return (
        <div className="flex md:flex-row flex-col h-[92vh] overflow-hidden w-full">
            <div className="w-[50%]  bg-primary hidden md:flex justify-center items-center">
                <img src={getImageIllUrl("login-ill.svg")} alt="login-illustration" className="w-10/12 h-80"/>
            </div>
                ...
                <LoginCard>
                    ...
                    <form action="" method="post" className="flex flex-col gap-3">
                        <div className="flex flex-col gap-2 justify-start">
                            ...
                            <span className={`${isEmailErr == true ? 'block' : 'hidden'} text-sm text-red-500 mt-[-5px]`}   id="emailMsg">Email harus diisi!</span>
                        </div>
                        ...
                        <p className="text-sm">
                            Forgot Password? click <Link to={"/auth/forgot-password"} className="underline text-blue-500">here</Link>
                        </p>
                        <div className="flex justify-center items-center gap-3">
                            ...
                            <button type="button" className={`flex ${isProcessLogin == false ? "bg-blue-500 active:translate-y-[2px] transition duration-75" : "bg-gray-400"}  text-white rounded-md p-2 items-center justify-center shadow-sm `} onClick={loginHandling} disabled={isProcessLogin}>
                                {isProcessLogin == false ? "Login" : <Loading classLoading={classLoading}/>}
                            </button>
                        </div>
                    </form>
                </LoginCard>
